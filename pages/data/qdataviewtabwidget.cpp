...
QDataViewTabWidget::QDataViewTabWidget(QWidget *parent) :
	QWidget(parent),
	m_currentIndex(-1),
...
QSize QDataViewTabWidget::minimumSizeHint() const
{
	return QSize(0, 25);
}

void QDataViewTabWidget::insert_data(QData *data, int index)
{
...
void QDataViewTabWidget::remove(QData *data)
{
...
int QDataViewTabWidget::current_index() const
{
	return m_currentIndex;
}

void QDataViewTabWidget::set_current_index(int index)
{
...
void QDataViewTabWidget::paintEvent(QPaintEvent *)
{
	...
	{
		QData *data = m_tabs.at(i);
		int w = fm.width(data->get_name());
		nameWidth << w;
		width += 2 * MARGIN + w;
	...
	for (int i = 0; i <= m_lastVisibleIndex; ++i)
	{
		...
		QData *data = m_tabs.at(actualIndex);
		...
		if (actualIndex == m_currentIndex)
		{
			...
			painter.fillRect(QRect(x, 0,
			                       2 * MARGIN + fm.width(data->get_name()),
			                       r.height() + 1),
			                 grad);
			...
			painter.drawText(x, baseline, data->get_name());
		...
		else
		{
			...
			painter.drawText(x + 1, baseline, data->get_name());
			x += nameWidth.at(actualIndex);
			x += MARGIN;
			drawSeparator(&painter, QPoint(x, 0), QPoint(x, r.height() - 1));
		}
...
void QDataViewTabWidget::mousePressEvent(QMouseEvent *event)
{
	if (!m_stop)
	{
...
QPair<QDataViewTabWidget::HitArea, int> QDataViewTabWidget::convertPosToTab(QPoint pos)
{
	if (pos.y() < StyleHelper::navigationWidgetHeight())
	{
		...
		{
			int otherX = x + 2 * MARGIN + fm.width(m_tabs.at(m_currentTabIndices.at(i))->get_name());
...
void QDataViewTabWidget::button_clicked()
{
	QMenu overflowMenu;
	...
	{
		actions << overflowMenu.addAction(m_tabs.at(m_currentTabIndices.at(i))->get_name());
	}
...
void QDataViewTabWidget::resizeEvent(QResizeEvent *e)
{
...
void QDataViewTabWidget::set_stop(bool stop)
{
	m_stop = stop;
