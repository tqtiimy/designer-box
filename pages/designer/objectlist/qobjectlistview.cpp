	...
	m_dialog_item(NULL)
{
	...
	connect(QSoftCore::get_core()->get_project_core()->get_page_manager(),
...
void QObjectListView::init()
{
	...
	while (list.size() > 0)
	{
		...
		{
			par = m_host_to_item.value(h->get_parent());
		}
...
void QObjectListView::insert_host_slot(const QList<QAbstractHost *> &list, const QList<int> &indexs)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		QList<QAbstractHost *> ch = h->get_children();

		while (ch.size() > 0)
		{
			QAbstractHost *hh = ch.takeFirst();
			QTreeWidgetItem *p = m_host_to_item.value(hh->get_parent());
...
void QObjectListView::host_parent_changed()
{
	...
	QTreeWidgetItem *item;
	item = m_host_to_item.value(host);
	QTreeWidgetItem *par = m_host_to_item.value(host->get_parent());
	item->parent()->removeChild(item);
	int index = host->get_parent()->get_children().indexOf(host);
...
void QObjectListView::button_remove()
{
	...
	while (it.hasNext())
	{
		...
		{
			QAbstractHost *par = it.key()->get_parent();
			...
			if (par != NULL)
			{
				...
				list.append(it.key());
				QList<int> indexs;
				indexs.append(par->get_children().indexOf(it.key()));
			...
			else
			{
				...
				new QPageAddUndoCommand(it.key(), manager->get_pages().indexOf(it.key()), PAT_REMOVE, cmd);

				QAbstractProperty *pro = QSoftCore::get_core()->get_project_core()->get_project_host()->get_property("start_page");

				if (pro != NULL)
				{
					if (pro->get_value().toString() == it.key()->get_uuid())
					{
						QList<QAbstractHost *> list = QSoftCore::get_core()
						                              ->get_project_core()->get_page_manager()->get_pages_by_title("form");
...
void QObjectListView::insert_page_slot(QAbstractHost *page)
{
	...
	int index = manager->get_pages_by_title(type).indexOf(page);
	...
	QList<QAbstractHost *> ch = page->get_children();
