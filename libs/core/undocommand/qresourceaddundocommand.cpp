...
{
	QSoftCore::get_core()->get_project_core()->get_resource_manager()->addResource(m_name, m_data);
}

void QResourceAddUndoCommand::remove()
{
	QSoftCore::get_core()->get_project_core()->get_resource_manager()->removeResource(m_name);
}
