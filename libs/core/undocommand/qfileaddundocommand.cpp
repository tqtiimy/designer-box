...
{
	QSoftCore::get_core()->get_file_manager()->insert_file(m_data.m_group_uuid, &m_data, m_index);
}

void QFileAddUndoCommand::remove()
{
	QSoftCore::get_core()->get_file_manager()->remove_file(m_data.m_group_uuid, m_data.m_uuid);
}
