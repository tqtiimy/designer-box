...
{
	QSoftCore::get_core()->get_project_core()->get_data_manager()->insert_group(m_data, m_index);
}

void QDataGroupAddUndoCommand::remove()
{
	QSoftCore::get_core()->get_project_core()->get_data_manager()->remove_group(m_data.get_uuid());
}
