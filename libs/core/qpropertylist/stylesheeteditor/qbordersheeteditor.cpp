...
void QBorderSheetEditor::take_resource(QUndoCommand *cmd)
{
	...
	{
		QResourceManager *manager = QSoftCore::get_core()->get_project_core()->get_resource_manager();
...
void QBorderSheetEditor::on_image_clicked()
{
	...
	tagResourceInfo *info = QSoftCore::get_core()->get_project_core()->get_resource_manager()->get_file()->resource(name);
	...
	if (dlg.get_ret() == 1)
	{
		...
		if (info != NULL)
		{
			tagFileGroupInfo *g = QSoftCore::get_core()->get_file_manager()->get_group(info->m_group_uuid);
			QString s = g->m_group_name + "/" + info->m_file_name;

			if (s != name)
			{
				QSoftCore::get_core()->get_project_core()
				->get_resource_manager()->addResource(s, info->m_file_data);
				m_item.m_attributes.insert("image", s);

				if (m_tempResource != "")
				{
					QSoftCore::get_core()->get_project_core()->get_resource_manager()->removeResource(m_tempResource);
				}
