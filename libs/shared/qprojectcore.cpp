...
bool QProjectCore::open(const QString &fileName)
{
	...
	{
		QString str = pro->get_value().toString();
		QList<QAbstractHost *> pages = m_page_manager->get_pages_by_title("form");
	...
	foreach(QAbstractHost * h, m_page_manager->get_pages())
	{
...
{
	if (form != NULL && (form->get_host_type() != "form" || form->get_parent() != NULL))
	{
		return;
	}
	...
	if (pro != NULL)
	{
		QList<QAbstractHost *> list = m_page_manager->get_pages_by_title("form");
		tagComboItem item;
		ComboItems items;
...
{
	QList<QAbstractHost *> list = m_page_manager->get_pages();
...
void QProjectCore::init_script_engine(QAbstractHost *host)
{
	...
	if (!keyboard || project)
	{
		...
		{
			temp = get_script_object(host->get_parent(), engine);
